<template>
  <div class="dishhotshop">
    
    <!--搜索-->
    <div class="searchDiv">
    	<input type="text" v-model='searchValue' placeholder="搜索菜名\食材" @input="searchMethod"/>
...
export default{
	...
	methods:{
		searchMethod(){//关键字为空时 搜索全部
			if(this.searchValue == ''){
		    this.getCaiPinList('UpdateDate','旺店热卖');//获取菜品列表 更新时间
		    this.getCaiPinList('RenQi','旺店热卖');//获取菜品列表 人气
			}
		},
		searchClick(){//点击搜索
			if(this.searchValue != ''){
...
//				self.dataArr = JSON.parse(res.data).data;
				self.dataArr = res.data.data;//测试
				
				for(let i in self.dataArr){
				...
				if(OrderBy == 'UpdateDate'){//更新时间
					self.dishUpdateList = self.dataArr;
					self.getCity1();//根据经纬度获取省市区名称
				}else if(OrderBy == 'RenQi'){//人气
					self.dishPopList = self.dataArr;
					self.getCity2();//根据经纬度获取省市区名称
				}
				
			})
		},
		getCity1(){//更新时间
			for(let i in this.dishUpdateList){
				var data = this.dishUpdateList[i];
				if(data.HotelAtitude!=''&&data.HotelAtitude!=null){
					this.getLocation(data.HotelAtitude,data.HotelLongitude,i,'time');//根据经纬度获取省市区名称
				}
			}
		},
		getCity2(){//人气
			for(let i in this.dishPopList){
				var data = this.dishPopList[i];
				if(data.HotelAtitude!=''&&data.HotelAtitude!=null){
					this.getLocation(data.HotelAtitude,data.HotelLongitude,i,'pop');//根据经纬度获取省市区名称
				}
			}
		},
		getLocation(latitude,longitude,index,OrderBy){//根据经纬度获取省市区名称
	    	var self = this;
	      $.ajax({
...
//	          console.log(city);
	          if(OrderBy == 'time'){
	          	self.dishUpdateList[index].city = city;
	            self.dishUpdateList.splice(index,1,self.dishUpdateList[index])
	          }else{
	          	self.dishPopList[index].city = city;
	            self.dishPopList.splice(index,1,self.dishPopList[index])
	          }
	        }
	      })
	  },
...
.box p:nth-child(2){font-size: 14px;color: #e83428;}
.box p:nth-child(3){font-size: 14px;color: #3E3E3E;}
.box p:nth-child(4) *{vertical-align: middle;}
.box p:nth-child(4) img{width: 15px;margin-right: 5px;}
.box p:nth-child(4) span{font-size: 12px;color: #CCCCCC;}

</style>
<style>
#vux_view_box_body{background: #fff;}
.dishhotshop .vux-tab .vux-tab-item{color: #3E3E3E;background: none;}
.dishhotshop .vux-tab .vux-tab-item.vux-tab-selected{color: #e91e24;}
.dishhotshop .vux-tab-bar-inner{background-color:#e91e24;height: 2px;position: relative;top: 1px;}
.dishhotshop .vux-tab{width: 50%; margin-left: 25%;}
</style>