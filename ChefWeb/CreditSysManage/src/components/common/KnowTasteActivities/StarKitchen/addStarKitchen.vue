	        			...
	        			<div class="left">
	        				...
	        				<ul style="display: block;float: left;margin-left: 1rem;">
	        					<li class="li">
	        						...
	        						<Input  placeholder="请输入姓名" v-model='KitchenName' style="width: 180px;margin-left: 1rem;"></Input>
	        					</li>
	        					<li class="li">
	        						<span style="margin-right: 1.6rem;">岗位</span>
	        						<Input  placeholder="请输入头衔"  v-model='jobs' style="width: 180px;margin-left: 1rem;"></Input>
	        					</li>
	        					<li class="li">
	        						<span style="margin-right: 1.6rem;">酒店</span>
	        						<Input  placeholder="请输入酒店" v-model="KitchenHotel" style="width: 180px;margin-left: 1rem;"></Input>
	        					</li>
	        					<li class="li">
	        						<span style="margin-right: 1.5rem;">城市</span>
	        						<Input  placeholder="请输入城市" v-model="KitchenCity" style="width: 180px;margin-left: 1rem;"></Input>
	        					</li>
	        					<li class="li">
	...
	export default{
		data(){
			return{
				...
				ingredientsOne:[],
				ingredientsTwo:[],
				starKitchenId:'',//保存基本信息后返回星厨id号(用于保存菜品
               //===================================================name2
                  content2: '',//存储编辑框
				 editorOption2: {
                 },
		...
		methods:{
			saveData(){//保存
				var self =this;
				if (!self.starKitchenId) {//此判断防止重复提交基本信息(每提交一次,生成新的人员id)
...
//						self.$Message.error('上传图片不能为空');
						...
						$('.loding').show()
						let DATA = '{"Introduce":"'+self.content+'","PicUrl":"'+self.bagImage+'","ChefStarName":"'+self.KitchenName+'","HeadPicUrl":"'+self.headerImage+'","ChefStarPosition":"'+self.jobs+'","HotelName":"'+self.KitchenHotel+'","CityName":"'+self.KitchenCity+'","IsDisplay":'+self.IsDisplay+',"PriorityId":'+self.PriorityId+',"CreatePerson":"'+self.sessionStorage+'","ChefActivityId":"'+self.chefactivityid+'"}'
						console.log(DATA)
						$.ajax({
							type:"post",
							url:URLHeader.ECActivities+'/api/ChefStar/AddChefStar',
							dataTape:'json',
...
//              return check;
			...
			saveDish(DishName,DishStory,DishPicUrl,DishType){//菜品保存
				...
				console.log('starKitchenId=',self.starKitchenId)
				//self.starKitchenId=1008;
				if (self.starKitchenId) {//基本信息已经保存
					if (DishPicUrl.indexOf('/static/image/')>0 || !DishName || !DishStory) {
						...
						var  DATA = '{"ChefStarId":"'+self.starKitchenId+'","DishName":"'+DishName+'","DishStory":"'+DishStory+'","DishPicUrl":"'+DishPicUrl+'","DishType":"'+DishType+'","CreatePerson":"张三","UpdatePerson":"'+self.sessionStorage+'"}'
						console.log(DATA)
						$.ajax({
							type:"post",
							url:URLHeader.ECActivities+'/api/ChefStar/AddDishChefStar',
							dataTape:'json',
							contentType:'application/json; charset=utf-8',
							data:DATA,
							success:function(json){
			...
			saveDishMaterial(DishType,DishId){//调料
				...
				if (t==0) {
					...
					$.ajax({
						type:"post",
						url:URLHeader.ECActivities+'/api/ChefStar/AddDishMaterial',
						dataTape:'json',
						contentType:'application/json; charset=utf-8',
						data:DATA,
						success:function(json){
			...
			saveOneStep(DishType,DishId){//步骤保存
				...
				if (t==0) {
					$.ajax({
						type:"post",
						url:URLHeader.ECActivities+'/api/ChefStar/AddDishStep',
						dataTape:'json',
						contentType:'application/json; charset=utf-8',
			...
			saverecommend(DishType,DishId){//推荐配料保存
				...
				if (DATA.length>0) {
					...
					$.ajax({
						type:"post",
						url:URLHeader.ECActivities+'/api/ChefStar/AddFR',
						dataTape:'json',
						contentType:'application/json; charset=utf-8',
		...
		mounted:function(){
			var self =this;
			//$('.addStarKitchen .rtf-content .ql-video').hide()//隐藏视频功能
			$('.addStarKitchen .name2 .rtf-content .ql-video').hide()//隐藏视频功能
			$('.addStarKitchen .name3 .rtf-content .ql-video').hide()//隐藏视频功能
			get_URL();
			$('.addStarKitchen .rtf-content .ql-container .ql-tooltip').css({//视频地址弹框移动
				'margin-left':'9rem'
			})
			function get_URL(){//获取地址参数
    				...
    				$.ajax({//获取星厨推荐配料
			        type :"get",
			        url :URLHeader.ECActivities+'/api/ChefStar/GetChefStarFlavourRec?caId='+hash,
			        dataType : 'json',
			        success : function(json) {
...
.addStarKitchen .name1{
		width:70rem;
		margin: 0 auto;
	}
.addStarKitchen .name1 .left,.addStarKitchen .name1 .right{
	float: left;
	width: 40rem;
}
.addStarKitchen .name1 .right{
}
.addStarKitchen .name1 .left{
	width: 30rem;
	min-height: 40rem;
	border-right:1px dashed gray ;
	padding-right: 1rem;
}
.addStarKitchen .name1 .left ul .li{
	margin-top: 1rem;
}
.addStarKitchen .name1 .right .rtf-content {
  ...
  /*name2*/
 .addStarKitchen .name2{
	width: 70rem;
	height: 100%;
	margin: 0 auto;
}
.addStarKitchen .name2 .left,.addStarKitchen .name2 .right{
	float: left;
}
.addStarKitchen .name2 .left{
	overflow: auto;
	height: 100%;
	width: 30rem;	
	
}
.addStarKitchen .name2 .right{
	width: 40rem;	
	border-left: 1px dashed gray;
}
.addStarKitchen .name2 .left .rtf-content {
    /*height: 380px;*/
    padding-right: 1rem;
    margin-left: 2rem;
  }
...
.Accessories  ul {
 ...
 /*=======name3*/
.addStarKitchen .name3{
	margin: 0 auto;
	width: 70rem;
	height: 100%;
}
.addStarKitchen .name3 .left,.addStarKitchen .name3 .right{
	float: left;
}
.addStarKitchen .name3 .left{
	overflow: auto;
	height: 100%;
	width: 30rem;	
	
}
.addStarKitchen .name3 .right{
	width: 40rem;	
	border-left: 1px dashed gray;
}
.addStarKitchen .name3 .left .rtf-content {
   /* height: 380px;*/
    padding-right: 1rem;
    margin-left: 2rem;
  }
