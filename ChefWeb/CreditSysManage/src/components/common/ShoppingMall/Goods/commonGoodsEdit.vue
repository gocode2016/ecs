<template>
	<div class="commonGoodsEdit">
		<Spin fix class='loding'>
            <Icon type="load-c" size=18 class="demo-spin-icon-load"></Icon>
            <div>Loading</div>
        </Spin>
		<h1 style="text-align: center;margin: 0rem auto;margin-top: 1rem;">普通商品编辑</h1>
		<ul style="overflow: auto;border-bottom:1px dashed gainsboro;">
			<li style="width: 14rem;" class="GoodsAddli">
					<img :src="GoodsInformation.ImgUrl" style="max-height: 5rem;max-width: 10rem;display: block;margin: 0 auto;margin-top: 0.6rem;"/>
					<Upload
				        ref="upload"
				        ...
				        style="display:block;width:90px;margin: 0 auto;margin-top: 0.5rem;margin-bottom: 0.5rem;" >
...
//				editorOption:{},
		...
		methods:{
            ...
            saveAlldata(){//整体保存    /cts/masll/api/Product/UpdateProduct
            		var self =this
            			//获取当前时间
	            		var myDate = new Date();
					//获取当前年
            ...
            requestGoodsInformation(){//初入界面数据请求
	   		    ...
	   		    $.ajax({//获取规格
			        type :"get",
			        url :URLHeader.ShoppingMall+'/api/Norms/GetSysProductNorms?productId='+self.GoodsInformation.goodId,
			        dataType : 'json',
			        contentType: "application/json; charset=utf-8",				
			        success : function(json) {
...
.commonGoodsEdit .ql-container{
	height: 80%;
}
.commonGoodsEdit .quill-editor{
	height: 22rem;
}
.commonGoodsEdit .rtf-content .ql-toolbar .ql-video{
	display: none;
}
</style>
<style scoped>
