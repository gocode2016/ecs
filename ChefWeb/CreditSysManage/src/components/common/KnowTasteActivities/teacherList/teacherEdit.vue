	        ...
	        <Tab-pane label="导师信息" name="name1" >
	        		<div class="name1" style="padding-top:1.5rem ;">
	        			<div class="left">
	        				...
	        				<ul style="display: block;float: left;margin-left: 1rem;position: relative;">
	        					<li class="li">
	        					...
	        					<li class="li">
	        						...
	        						<Input  placeholder="请输入头衔"  v-model='TutorPosition' style="width: 180px;margin-left: 1rem;"></Input>
	        					</li>
	        					<li class="li">
	        ...
	        <Tab-pane label="菜品1" name="name2">
	        		<div class="name2">
	        			...
	        			<div class="right">
	        				...
	        				<Button type="primary" @click='saveDish(1,TutorDishId1)' style="margin-left: 22rem;margin-top: 1rem;">菜品保存</Button>
	        			</div>
	        		</div>
	        </Tab-pane>
	        <Tab-pane label="菜品2" name="name3">
	        		<div class="name3">
	        			...
	        			<div class="right">
	        				...
	        				<Button type="primary" @click='saveDish(2,TutorDishId2)' style="margin-left: 22rem;margin-top: 1rem;">菜品保存</Button>
	        			</div>
	        		</div>
	        </Tab-pane>
	   </Tabs>
...
//                  },
		...
		methods:{
			...
			saveData(){//基本信息保存    Tutor/UpdateSingle
				$('.loding').show()
				var self =this;
				self.content = self.content.replace(/\"/g,"'");//把双引号改为单引号
				//self.content=self.content.replace(/\"/g, "\\\'"); 	
				//self.content=JSON.stringify(self.content)
				var Area = self.AreaName.join('|')
				let DATA = '{"ChefActivityId":'+self.ChefActivityId+',"TutorId":"'+self.TutorId+'","Introduce":"'+self.content+'","PicUrl":"'+self.bagImage+'","TutorName":"'+self.TutorName+'","HeadPicUrl":"'+self.headerImage+'","AreaName":"'+Area+'","TutorPosition":"'+self.TutorPosition+'","TutorHotel":"'+self.TutorHotel+'","PriorityId":"'+self.PriorityId+'","IsDisplay":'+self.IsDisplay+',"UpdatePerson":"'+self.sessionStorage+'"}'
				console.log(DATA)
				$.ajax({
					type:"post",
					url:URLHeader.ECActivities+'/api/Tutor/UpdateSingle',
					dataTape:'json',
...
//              return check;
	         ...
	         getTeacherInformation(){//获取导师基本信息
				var self =this;
//				self.TutorId= 61;
				$.ajax({//获取活动名称
			        type :"get",
...
//			        		console.log(json)
			        		...
			        		//console.log(dataAll)
				        		...
				        		//新增模式
				        		if (DishType=='1') {//菜品一
				        		self.TutorDishId1=0;  //菜品1id
				        		} else{//菜品二
				        			self.TutorDishId2=0;  //菜品2id
				        		}
...
//	         	console.log(data)
	         	...
	         	self.TutorDishId1=Basic.TutorDishId;  //菜品1id
	         ...
	         dishTwoParsing(data){//解析菜品2
	         	...
	         	self.TutorDishId2=Basic.TutorDishId;  //菜品2id
	         	for (var i=0;i<data.Select.length;i++) {
			...
			saveDish(dishType,TutorDishId){//菜品1保存
				$('.loding').show()
				var self =this
				var DATA;
				var arr=[];
				...
				if (dishType=='1') {//菜品1
					...
					//../../../../../static/image/placeholderBag.png
					var DishPicUrl = ''
					if (self.name2Image =='../../../../../static/image/placeholderBag.png') {
						DishPicUrl = ''
					}else{
						DishPicUrl = self.name2Image
					}
					flavourRecRole = JSON.stringify(flavourRecRole)
					DATA='{"dishTutor":{"TutorDishId":"'+self.TutorDishId1+'","TutorId":"'+self.TutorId+'","DishName":"'+self.dishName1+'","DishStory":"'+self.content2+'","DishPicUrl":"'+DishPicUrl+'","DishType":1,"CreatePerson":"张三","UpdatePerson":"'+self.sessionStorage+'"},"dishMaterial":'+dishMaterial+',"dishStep":'+DishStep+',"flavourRecRole":'+flavourRecRole+'}'
				} else{
					...
					for (var t = 0;t<self.ingredientsTwo.length;t++) {
						var item = {
							...
							DishId:self.TutorDishId2
						}
						flavourRecRole.push(item)
					}
					var DishPicUrl = ''
					if (self.name3Image =='../../../../../static/image/placeholderBag.png') {
						DishPicUrl = ''
					}else{
						DishPicUrl = self.name3Image
					}
					flavourRecRole = JSON.stringify(flavourRecRole)
					DATA='{"dishTutor":{"TutorDishId":"'+self.TutorDishId2+'","TutorId":"'+self.TutorId+'","DishName":"'+self.dishName2+'","DishStory":"'+self.content3+'","DishPicUrl":"'+DishPicUrl+'","DishType":2,"CreatePerson":"张三","UpdatePerson":"'+self.sessionStorage+'"},"dishMaterial":'+dishMaterial+',"dishStep":'+DishStep+',"flavourRecRole":'+flavourRecRole+'}'
				}
				console.log(DATA)
				$.ajax({
					type:"post",
					url:URLHeader.ECActivities+'/api/DishTutor/UpdateDishTutor',
					dataTape:'json',
					contentType:'application/json; charset=utf-8',
					data:DATA,
					success:function(json){
		...
		mounted:function(){
			var self =this;
			$('.teacherEdit .rtf-content .ql-video').hide()//隐藏视频功能
			$('.teacherEdit .rtf-content .ql-container .ql-tooltip').css({//视频地址弹框移动
				'margin-left':'9rem'
			})
			get_URL();
			self.getTeacherInformation();//导师基本信息
...
.teacherEdit .name1 .left,.teacherEdit .name1 .right{
	float: left;
	width: 40rem;
}
.teacherEdit .name1 .right{
	/*height: 100%;*/
	border-left:1px dashed gray ;
}
.teacherEdit .name1 .left{
	padding-right: 1rem;
	width: 30rem;
	min-height: 40rem;
}
.teacherEdit .name1 .left ul .li{
	margin-top: 1rem;
}
.teacherEdit .name1 .right .rtf-content {
    /*height: 38rem;*/
    width: 33rem;
    margin-left: 1rem;
  }
...
.teacherEdit .name2 .left,.teacherEdit .name2 .right{
	float: left;
}
.teacherEdit .name2 .left{
	width: 30rem;	
	height: 100%;
}
.teacherEdit .name2 .right{
	width: 40rem;	
	border-left: 1px dashed gray;
}
.teacherEdit .name2 .left .rtf-content {
...
.teacherEdit .name3 .left,.teacherEdit .name3 .right{
	float: left;
}
.teacherEdit .name3 .left{
	height: 100%;
	width: 30rem;	
	
}
.teacherEdit .name3 .right{
	width: 40rem;	
	border-left: 1px dashed gray;
}
.teacherEdit .name3 .left .rtf-content {
   /* height: 30rem;*/
    padding-right: 1rem;
    margin-left: 2rem;
  }
