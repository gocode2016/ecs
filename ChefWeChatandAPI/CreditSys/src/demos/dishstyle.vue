...
export default{
	...
	methods:{
		searchMethod(){//关键字为空时 搜索全部
			if(this.searchValue == ''){
		    this.getCaiPinList('UpdateDate',this.secondId);//获取菜品列表 更新时间
		    this.getCaiPinList('RenQi',this.secondId);//获取菜品列表 人气
			}
		},
		searchClick(){//点击搜索
			if(this.searchValue != ''){
		...
		getCaiPinList(OrderBy,secondId){//获取菜品列表
			var self = this;
			var params = {
		    "page": 1,
		    "pagesize": 1000,
...
.dishstyle .vux-tab .vux-tab-item{color: #3E3E3E;background: none;}
.dishstyle .vux-tab .vux-tab-item.vux-tab-selected{color: #e91e24;}
.dishstyle .vux-tab-bar-inner{background-color:#e91e24;height: 2px;position: relative;top: 1px;}
.dishstyle .vux-tab{width: 50%; margin-left: 25%;}
</style>